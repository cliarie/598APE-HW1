...
MAIN_DATA->addLight(std::make_unique<Light>(
    ...
    texture, yaw, pitch, roll, tx, ty
);
Plane* shapePtr = shape.get();  // Save the raw pointer
MAIN_DATA->addShape(std::move(shape));  // Transfer ownership
shapePtr->normalMap = parseTexture(f, true);
            ...
            Texture *texture = parseTexture(f, false);
        auto shape = std::make_unique<Disk>(
    Vector(disk_x, disk_y, disk_z),
    texture, yaw, pitch, roll, tx, ty
);
Disk* shapePtr = shape.get();  // Save the raw pointer for later access.
MAIN_DATA->addShape(std::move(shape));  // Transfer ownership.
shapePtr->normalMap = parseTexture(f, true);
            ...
            // shape->normalMap = parseTexture(f, true);
            // 
auto shape = std::make_unique<Box>(
    Vector(box_x, box_y, box_z),
    texture, yaw, pitch, roll, tx, ty
);
Box* shapePtr = shape.get();  // Save the raw pointer for later access.
MAIN_DATA->addShape(std::move(shape));  // Transfer ownership.
shapePtr->normalMap = parseTexture(f, true);
...
Triangle* shapePtr = shape.get();  // Save the raw pointer for later use.
MAIN_DATA->addShape(std::move(shape));  // Transfer ownership.
shapePtr->normalMap = parseTexture(f, true);
...
Sphere* shapePtr = shape.get();  // Save a raw pointer for later access.
MAIN_DATA->addShape(std::move(shape));  // Transfer ownership to MAIN_DATA.
shapePtr->normalMap = parseTexture(f, true);
...
Triangle* shapePtr = shape.get();  // Save a raw pointer to modify later.
MAIN_DATA->addShape(std::move(shape));
shapePtr->normalMap = normalMap;
...
//             }
//             Texture *texture = parseTexture(f, false);
//             std::unique_ptr<Disk> shape = std::make_unique<Disk>(
//                 Vector(disk_x, disk_y, disk_z),
//                 texture,
//                 yaw, pitch, roll, tx, ty
//             );
...
//             }
//             Texture *texture = parseTexture(f, false);
//             std::unique_ptr<Box> shape = std::make_unique<Box>(
//                 Vector(box_x, box_y, box_z),
//                 texture,
//                 yaw, pitch, roll, tx, ty
//             );
...
//             }
//             Texture *texture = parseTexture(f, false);
//             std::unique_ptr<Triangle> shape = std::make_unique<Triangle>(
//                 Vector(x1, y1, z1),
//                 Vector(x2, y2, z2),
//                 Vector(x3, y3, z3),
//                 texture
//             );
//             Triangle* shapeptr = shape.get();
//             // Triangle* shape = new Triangle(Vector(x1, y1, z1), Vector(x2, y2, z2), Vector(x3, y3, z3), texture);
//             MAIN_DATA->addShape(std::move(shape));
//             shapeptr->normalMap = parseTexture(f, true);
//          } else if (streq(object_type, "sphere")) {
...
//                node = node->next.get();
//             }
//             std::unique_ptr<Shape> shape = std::move(node->data);
//
//             if (streq(field_type, "yaw")) {
//                shape->setYaw(result);
//             } else if (streq(field_type, "pitch")) {
